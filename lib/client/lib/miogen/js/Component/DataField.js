        ...
        build: function (cb) {
            ...
            switch (dataType) {
                case 'boolean': {
                    if (required) {
                        ...
                        this.field = new Miogen.Component.Dropdown({
            ...
            Miogen.bind(t, this.data.getItem(), 'onValueChange', function (attribute, value) {
                if (attribute === t.data.getAttribute() + '.value') {
                    ...
                    if (t.overrideDefaultCheckbox !== null) {
                        if (value === null && t.overrideDefaultCheckbox.attr('value') === true) {
                            t.overrideDefaultCheckbox.attr('value', false); // We are going back to the not overriding
                            t.field.attr('disabled', true);
                            t.field.attr('value', t.data.getInheritedValue());
                        }
                        else {
                            if (value !== null && !t.overrideDefaultCheckbox.attr('value')) {
                                // We are overriding the value
                                t.field.attr('disabled', false);
                                t.overrideDefaultCheckbox.attr('value', true);
                            }
                            
                            // Only set the value if it's different and is not null (as the text box value will be the inherited one)
                            if (t.field.attr('value') !== value && (t.overrideDefaultCheckbox.attr('value') !== false && value !== null)) {
                                t.field.attr('value', value);
                            }
                        }
                    }
                    else {
                        if (t.field.attr('value') !== value) {
                            t.field.attr('value', value);
                        }
                    }
                }
            ...
            if (this.overrideDefaultCheckbox !== null) {
                ...
                if (this.data.getValue() === null) {
                    ...
                    this.field.attr('value', t.data.getInheritedValue());
                }
